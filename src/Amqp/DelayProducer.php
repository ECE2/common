...
class DelayProducer extends Producer
{
    public function produce(
        ProducerMessageInterface $producerMessage,
        bool $confirm = false,
        int $timeout = 5,
        int $delayTime = 0
    ): bool
    ...
    private function produceMessage(ProducerMessageInterface $producerMessage, bool $confirm = false, int $timeout = 5, int $delayTime = 0): bool
    {
        ...
        // 触发队列发送之前事件
        event(new BeforeProduce($producerMessage, $delayTime));
        ...
        //触发队列发送之中事件
        event(new ProduceEvent($producerMessage));
        ...
        try {
            ...
            // 触发队列发送失败事件
            event(new FailToProduce($producerMessage, $exception));
        ...
        // 触发队列发送之后事件
        event(new AfterProduce($producerMessage));
        return $result;
    }
