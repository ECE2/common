...
class Adapter implements FilesystemAdapter
{
    ...
    {
        $endpoint = trim(trim($this->config['endpoint']), '/');
        if ($this->config['isCName'] ?? false) {
            $domain = $endpoint;
        } else {
            $domain = $this->bucket . '.' . $this->checkEndpoint($endpoint);
        }
    ...
    private function checkEndpoint($endpoint)
    {
        ...
        return OssUtil::getHostPortFromEndpoint($retEndpoint);
    }
